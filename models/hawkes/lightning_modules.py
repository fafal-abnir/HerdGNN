...
class LightningNodeGNN(L.LightningModule):
    ...
    def forward(self, data):
        ...
        pred, anomaly_score, embeddings = self.model(x, edge_index, edge_ages)
        return pred, anomaly_score, embeddings
...
class LightningEdgeGNN(L.LightningModule):
    def __init__(self, model, learning_rate, alpha: float = 0.0, anomaly_loss_margin: float = 4.0,
                 blend_factor: float = 1.0):
        super().__init__()
        self.model = model
        self.learning_rate = learning_rate
    ...
    def forward(self, data):
        ...
        edge_ages = data.edge_ages
        pred, anomaly_scores, embeddings = self.model(x, edge_index, edge_label_index, edge_attr, edge_ages)

...
class NodeHERDNet(nn.Module):
    def __init__(self, input_dim, memory_size=16, hidden_size=16, out_put_size=2, enable_memory=True, gnn_type="GCN",
        ...
        for i in range(num_layers):
            ...
            if self.bns is not None: self.bns.append(nn.BatchNorm1d(hidden_size))
...
class EdgeHERDNet(nn.Module):
    def __init__(self, input_dim, edge_attr_dim, memory_size=16, hidden_size=16, enable_memory=True, gnn_type="GCN",
        ...
        self.convs = nn.ModuleList()
        if use_bn:
            self.bn_pre1 = nn.BatchNorm1d(hidden_size)
            self.bn_pre2 = nn.BatchNorm1d(hidden_size)
        self.bns = nn.ModuleList() if use_bn else None
...
class MemoryAugmentedGNN(nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        if self.gnn_type == "GAT":
            ...
            for i in range(self.heads):
                self.gnn.att_l[i].weight = nn.Parameter(new_weights[i].detach())  # Update GAT attention

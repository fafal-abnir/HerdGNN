...
from models.wingnn.lightning_modules import LightningNodeGNN, LightningEdgeGNN
...
def get_args():
    ...
    parser.add_argument("--sliding_window_size", type=int, default=6, help="Sliding window size")
    parser.add_argument("--train_wingnn_window_size", type=int, default=4,
                        help="Size of window size for doing frame wise and window wise gradient")
    ...
    parser.add_argument("--graph_window_size", type=str, choices=["day", "week", "month"], default="week",
...
def main():
    ...
    if dataset_name in ["DGraphFin", "EllipticPP", "EthereumPhishing"]:
        task = "Node"
        lightning_root_dir = "experiments/wingnn/node_level"
        if dataset_name == "EllipticPP":
            graph_window_size = "hour"
    else:
        task = "Edge"
        lightning_root_dir = "experiments/wingnn/edge_level"
    ...
    for data_index in range(len(dataset) - sliding_window_size - 1):
        snapshot = dataset[data_index]
        if snapshot.x is None:
            snapshot.x = torch.Tensor([[1] for _ in range(snapshot.num_nodes)])
        if snapshot.x is None:
            snapshot.x = torch.Tensor([[1] for _ in range(snapshot.num_nodes)])
        experiments_dir = f"{lightning_root_dir}/{dataset_name}/{graph_window_size}/" \
                          f"{gnn_type}_S{sliding_window_size}_T{train_wingnn_window_size}/" \
                          f"{experiment_datetime}/index_{data_index}"
        ...
        if task == "Node":
            ...
            train_data_list = dataset[data_index:data_index + sliding_window_size]
            val_data = dataset[sliding_window_size]
            test_data = dataset[sliding_window_size + 1]

            test_data.num = test_data.num_nodes
            if snapshot.x is None:
                test_data.x = torch.Tensor([[1] for _ in range(test_data.num_nodes)])
            if model is None:
                model = NodeGNN(snapshot.x.shape[1], hidden_size=hidden_size,
            ...
            train_loader = DataLoader(train_data_list, batch_size=len(train_data_list))
            ...
            del train_data_list, train_loader, val_data, val_loader, test_data, test_loader, snapshot
            gc.collect()
            torch.cuda.empty_cache()

        else:
            train_data_list = []
            for i, data in enumerate(dataset[data_index:data_index + sliding_window_size]):
                data.edge_label_index = data.edge_index
                train_data_list.append(data)
            val_data = copy.deepcopy(dataset[sliding_window_size])
            val_data.edge_label_index = val_data.edge_index
            test_data = copy.deepcopy(dataset[sliding_window_size + 1])
            test_data.edge_label_index = test_data.edge_index

            if snapshot.x is None:
                test_data.x = torch.Tensor([[1] for _ in range(test_data.num_nodes)])
            if model is None:
                model = EdgeGNN(snapshot.x.shape[1], edge_attr_dim=dataset.num_edge_features,
